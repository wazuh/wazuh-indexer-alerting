...
# ====
# Sync the hardcoded version fallback(s) in build.gradle to the release version.
...
# alerting has two such fallbacks (artifactVersion + wazuh_version); the per-line
# sed rewrites both.
# Arguments:
#   $1 - version
# ====
function update_build_gradle_version() {
    ...
    log "Synced $file hardcoded version fallback(s) to $version"
}
