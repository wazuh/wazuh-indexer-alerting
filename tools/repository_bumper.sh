...
# falls back to reading VERSION.json when it is not passed (e.g. CodeQL autobuild,
# which runs a bare gradle with no -Dversion), so VERSION.json is the single source
# of truth and there is nothing for this script to sync. A hardcoded literal coming
# back means the fallback can drift behind VERSION.json again and request a
# common-utils artifact (${wazuh_version}.${revision}) that was never published, so
# fail rather than warn. alerting used to carry two such fallbacks (artifactVersion
# + wazuh_version), so the check is not anchored to a single line.
# ====
